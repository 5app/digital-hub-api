...
function extractUserData({rawUserData, forbiddenTeams = []}) {
	...
	rawUserData.forEach(user => {
		...
		const teams = [user.jobFamilyTeam.trim(), user.fullJobTitleTeam.trim(), user.countryTeam.trim()]
		...
		teams.forEach(team => {
			...
			if (forbiddenTeams.includes(teamName)) {
				usersWithForbiddenTeams[teamName] = usersWithForbiddenTeams[teamName] || new Set()
	...
	Object.keys(allTeams).forEach(name => {
		...
		if (teamNameVariations.size > 1) { // the same name was written in 2 or more variations
	...
	const doNotImportAccounts = invalidaEmails.length
